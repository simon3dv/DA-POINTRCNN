...
def model_joint_fn_decorator():
    ...
    def print_dict(data,name):
        ...
        for key in data.keys():
            if isinstance(data[key], np.ndarray) or isinstance(data[key], torch.Tensor):
    ...
    def model_fn(model, data):
        ...
        if cfg.DA.ENABLED:
            if cfg.RPN.ENABLED and not cfg.RPN.FIXED and cfg.DA.DA_IMG.ENABLED:
                ...
                disp_dict['da_rpn_acc'] = (da_rpn_output.eq(torch.FloatTensor(is_source).cuda())).float().mean()
            if cfg.RCNN.ENABLED and cfg.DA.DA_INS.ENABLED:
                ...
                disp_dict['da_rcnn_acc'] = (da_rcnn_output.eq(torch.FloatTensor(is_source_for_rois).cuda())).float().mean()
    ...
    def get_da_rpn_loss(da_img, is_source, tb_dict):
        """
        :param da_img: B,1,N or B,1,1(with avg_pool) or B,1,1(with reshape)
        :param is_source: B,
        """
