...
class nuscenes2kittiRCNNDataset(nuscenes2kittiDataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='Car', mode='TRAIN', random_select=True,
        ...
        if classes == 'Car':
            ...
            aug_scene_root_dir = os.path.join(root_dir, 'nuScenes2KITTI', 'aug_scene')
        elif classes == 'People':
            self.classes = ('Background', 'Pedestrian', 'Cyclist')
        elif classes == 'Pedestrian':
            self.classes = ('Background', 'Pedestrian')
            aug_scene_root_dir = os.path.join(root_dir, 'nuScenes2KITTI', 'aug_scene_ped')
        elif classes == 'Cyclist':
            self.classes = ('Background', 'Cyclist')
            aug_scene_root_dir = os.path.join(root_dir, 'nuScenes2KITTI', 'aug_scene_cyclist')
        else:
    ...
    def get_rpn_sample(self, index):
        ...
        if sample_id < id_max:
            ...
            pts_global = calib.project_lidar_to_global(pts_lidar[:, 0:3].T)
        ...
        pts_img, pts_rect_depth = calib.project_cam_to_image(pts_rect.T, self.sensor, True)

        pts_img = pts_img.T
        pts_rect_depth = pts_rect_depth.T
        ...
        if self.mode == 'TRAIN' or self.random_select:
            if self.npoints < len(pts_rect):
                ...
                if self.npoints > len(pts_rect):
                    extra_choice = np.random.choice(choice, self.npoints - len(pts_rect), replace=True)###False)
