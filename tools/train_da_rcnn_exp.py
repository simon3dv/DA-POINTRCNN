...
import lib.net.train_functions_da_exp as train_functions
...
def create_dataloader(logger):
    ...
    # create dataloader
    source_train_set = KittiRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS, split=cfg.DA.SOURCE.SPLIT,
                                        mode='TRAIN',
                                        logger=logger,
                                        classes=cfg.CLASSES,
                                        ...
                                        is_source=True)
    source_train_loader = DataLoader(source_train_set, batch_size=args.batch_size, pin_memory=True,
                                     num_workers=args.workers, shuffle=True, collate_fn=source_train_set.collate_batch,
                                     drop_last=True)
    ...
    if cfg.DA.ENABLED:
        target_train_set = nuscenes2kittiRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS, split=cfg.DA.TARGET.SPLIT,
                                            ...
                                            gt_database_dir=args.gt_database,
                                            is_source=False)
        target_train_loader = DataLoader(target_train_set, batch_size=args.batch_size, pin_memory=True,
                                         num_workers=args.workers, shuffle=True, collate_fn=target_train_set.collate_batch,
                                         drop_last=True)

    if args.train_with_eval:
        source_test_set = KittiRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS,
                                    split=cfg.DA.SOURCE.VAL_SPLIT, mode='EVAL',
                                    logger=logger,
                                    classes=cfg.CLASSES,
...
if __name__ == "__main__":
    ...
    source_train_loader, target_train_loader, source_test_loader, target_test_loader = create_dataloader(logger)
    model = GeneralizedPointRCNN(num_classes=source_train_loader.dataset.num_class, use_xyz=True, mode='TRAIN')
    ...
    lr_scheduler, bnm_scheduler = create_scheduler(optimizer, total_steps=len(source_train_loader) * args.epochs,
